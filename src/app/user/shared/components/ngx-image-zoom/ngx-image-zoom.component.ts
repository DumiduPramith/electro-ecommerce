...
@Component({
  selector: 'app-ngx-image-zoom',
  templateUrl: './ngx-image-zoom.component.html',
  styleUrls: ['./ngx-image-zoom.component.scss'],
})
export class NgxImageZoomComponent implements OnInit, OnChanges, OnDestroy {
  ...
  // detect screen resize and change magnification ratio
  tempMagnification = 0
  @HostListener('window:resize', ['$event'])
  onResize() {
    let width = this.imageThumbnail.nativeElement.offsetWidth
    let original_width = this.imageThumbnail.nativeElement.naturalWidth
    this.magnification = (this.tempMagnification * width) / original_width
  }
  // end detect screen resize and change magnification ratio
  ...
  ngOnChanges() {
    ...
    this.tempMagnification = this.magnification
  }
    ...
    switch (this.zoomMode) {
      case 'hover':
        this.eventListeners.push(
          this.renderer.listen(nativeElement, 'mouseenter', (event) =>
            this.hoverMouseEnter(event)
          ),
          this.renderer.listen(nativeElement, 'mouseleave', () =>
            this.hoverMouseLeave()
          ),
          this.renderer.listen(nativeElement, 'mousemove', (event) =>
            this.hoverMouseMove(event)
          )
        )
        break
      case 'toggle':
        this.eventListeners.push(
          this.renderer.listen(nativeElement, 'click', (event) =>
      ...
      case 'hover-freeze':
        this.eventListeners.push(
          this.renderer.listen(nativeElement, 'mouseenter', (event) =>
            this.hoverFreezeMouseEnter(event)
          ),
          this.renderer.listen(nativeElement, 'mouseleave', () =>
            this.hoverFreezeMouseLeave()
          ),
          this.renderer.listen(nativeElement, 'mousemove', (event) =>
            this.hoverFreezeMouseMove(event)
          ),
          this.renderer.listen(nativeElement, 'click', (event) =>
            this.hoverFreezeClick(event)
          )
        )
    }
